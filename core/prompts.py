...
# 交易闸门（必须优先于量价评分）
输入中每只股票若出现 [交易闸门]：
- 二次确认:confirmed / 来源含“二次确认” / signal_status=confirmed，才允许进入“处于起跳板”。
- 二次确认:unconfirmed / pending / 空值，只能进入“储备营地”或“逻辑破产”，不得写入“处于起跳板”。
- 未确认但量价接近买点时，写成“近就绪”，说明还缺什么确认，不得给买入执行计划。
这是回测后的交易风格约束，不得用板块热度、政策主题或模型主观判断绕过。
